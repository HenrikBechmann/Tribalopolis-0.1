        ...
        this.createcontext = () => {
            ...
            for (let n = 1; n < datastack.length; n++) {
                ...
                if (itemProxy.liststack.length) { // make list entry
                    ...
                    let component = <DirectoryBar key={n + 'list'} haspeers={false} listProxy={listProxy} setDocpackPairListener={this.props.callbacks.setDocpackPairListener} removeDocpackPairListener={this.props.callbacks.removeDocpackPairListener} callDataDrawer={this.props.callDataDrawer} listStack={itemProxy.liststack} collapseDirectoryItem={() => { }} contextitem/>;
                    ...
                    context.push(<BoxIdentityBar key={n + 'item'} itemProxy={newItemProxy} setDocpackPairListener={this.props.callbacks.setDocpackPairListener} removeDocpackPairListener={this.props.callbacks.removeDocpackPairListener} callDataDrawer={this.props.callDataDrawer} contextitem/>);
                    let holderItemProxy = new docproxy({
                        doctoken: itemProxy.doctoken,
                        liststack: itemProxy.liststack.slice(),
                    });
                    let component = <RootDirectoryBarHolder key={n + 'list'} itemProxy={holderItemProxy} setDocpackPairListener={this.props.callbacks.setDocpackPairListener} removeDocpackPairListener={this.props.callbacks.removeDocpackPairListener} callDataDrawer={this.props.callDataDrawer}/>;
                    context.push(<Icon key={n + 'icon'} style={{ opacity: .54 }}>chevron_right</Icon>);
                    context.push(component);
                }
            }
